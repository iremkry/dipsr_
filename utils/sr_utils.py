...
def load_LR_HR_imgs_sr(fname, imsize, enforse_div32=None):
    ...
    print('HR and LR resolutions: %s' % (str(img_HR_pil.size)))
...
def get_baselines(img_HR_pil, img_orig_pil):
    '''Gets `bicubic`, sharpened bicubic and `nearest` baselines.'''
    img_bicubic_pil = img_HR_pil.resize(img_orig_pil.size, Image.BICUBIC)
    img_bicubic_np = pil_to_np(img_bicubic_pil)

    img_nearest_pil = img_HR_pil.resize(img_orig_pil.size, Image.NEAREST)
